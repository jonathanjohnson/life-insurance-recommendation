export type StateMeta = { abbr: string; name: string; fips: string };
...
const BY_ABBR = new Map(STATES.map((s) => [s.abbr, s] as const));
const BY_FIPS = new Map(STATES.map((s) => [s.fips, s] as const));

export function stateByAbbr(abbr: string): StateMeta | undefined {
  return BY_ABBR.get(abbr.toUpperCase());
}

export function stateByFips(fips: string): StateMeta | undefined {
